...
class LCASMainGUI(ctk.CTk):
    ...
    def _initialize_core_async(self):
        ...
        def run_core_init():
            ...
            self.core_app = LCASCore.create_with_config(config_path_str=str(config_path), main_loop=self.async_event_loop)
    ...
    def _create_config_setup_panel(self):
        ...
        # AI Settings Panel (Will be initialized after core is ready)
    ...
    def _initialize_ai_panel(self):
        """Initialize AI settings panel after core is ready"""
        if self.core_app and self.ai_settings_panel is None:
            # Find the config panel and add AI settings to it
            config_panel = self.panels.get("config_setup")
            if config_panel:
                self.ai_settings_panel = AIIntegrationPanel(config_panel, self)
